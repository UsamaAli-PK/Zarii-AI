...
(function () {
  ...
  function getToken() {
    return localStorage.getItem("zarii_token");
  }
  function getAdminToken() {
    return localStorage.getItem("zarii_admin_token");
  }
  ...
  window.API = {
    ...
    submitFeedback: (scanId, feedback) =>
      post("/api/diagnose/" + scanId + "/feedback", { feedback }),

    // ── History ───────────────────────────────────────────────
    getHistory: (params) => get("/api/history", params),
    getRecentScans: (limit) =>
      get("/api/history/recent", { limit: limit || 4 }),
    getAnalytics: (period) =>
      get("/api/history/analytics", period ? { period } : {}),

    // ── Voice ─────────────────────────────────────────────────
    transcribeAudio: (formData) =>
      post("/api/voice/stt", formData, false, true),
    askQuestion: (text, lang) => post("/api/voice/ask", { text, lang }),
    textToSpeech: (text, lang, queryId) =>
      post("/api/voice/tts", { text, lang, query_id: queryId }),
    ...
    adminFlagDiagnosis: (id) =>
      post("/api/admin/diagnoses/" + id + "/flag", {}, true),
    adminAccuracyChart: () =>
      get("/api/admin/diagnoses/meta/accuracy-chart", null, true),

    // ── Admin Outbreaks ───────────────────────────────────────
    adminOutbreaks: () => get("/api/admin/outbreaks", null, true),
    adminAdvisories: () => get("/api/admin/outbreaks/advisories", null, true),
    adminSendAdvisory: (data) =>
      post("/api/admin/outbreaks/advisories", data, true),

    // ── Admin Sponsors ────────────────────────────────────────
    adminSponsors: () => get("/api/admin/sponsors", null, true),
    adminAddSponsor: (data) => post("/api/admin/sponsors", data, true),
    adminUpdateSponsor: (id, data) =>
      patch("/api/admin/sponsors/" + id, data, true),
    adminSponsoredProducts: () =>
      get("/api/admin/sponsors/products", null, true),
    adminAddSponsoredProduct: (data) =>
      post("/api/admin/sponsors/products", data, true),
    adminUpdateSponsoredProduct: (id, data) =>
      patch("/api/admin/sponsors/products/" + id, data, true),
    ...
    adminUpdateProduct: (id, data) =>
      patch("/api/admin/catalog/" + id, data, true),
    adminDeleteProduct: (id) => del("/api/admin/catalog/" + id, true),
    adminBulkRefreshPrices: () =>
      post("/api/admin/catalog/bulk-refresh", {}, true),

    // ── Admin API Keys ────────────────────────────────────────
    adminApiKeys: () => get("/api/admin/api-keys", null, true),
    adminAddApiKey: (data) => post("/api/admin/api-keys", data, true),
    adminUpdateApiKey: (id, data) =>
      patch("/api/admin/api-keys/" + id, data, true),
    adminDeleteApiKey: (id) => del("/api/admin/api-keys/" + id, true),
    adminTestApiKey: (id) =>
      post("/api/admin/api-keys/" + id + "/test", {}, true),
    adminFailoverLog: () => get("/api/admin/api-keys/failovers", null, true),

    // ── Admin WhatsApp ────────────────────────────────────────
    adminWaQueue: () => get("/api/admin/whatsapp/queue", null, true),
    adminWaTakeover: (convId) =>
      post("/api/admin/whatsapp/takeover", { conversation_id: convId }, true),
    adminWaSend: (phone, msg, convId) =>
      post(
        "/api/admin/whatsapp/send",
        { wa_phone: phone, message: msg, conversation_id: convId },
        true,
      ),
    ...
    adminChangeRole: (id, role) =>
      patch("/api/admin/team/" + id + "/role", { role }, true),
    ...
    clearToken: () => {
      localStorage.removeItem("zarii_token");
      localStorage.removeItem("zarii_admin_token");
    },
    isLoggedIn: () => !!getToken(),
    isAdminLoggedIn: () => !!getAdminToken(),
    joinWaitlist: (userId, feature = "whatsapp", contact) =>
      post("/api/waitlist", { user_id: userId, feature, contact }),
  };
