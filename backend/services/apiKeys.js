...
const fetchKeys = async () => {
  ...
  try {
    const { data, error } = await supabase.from('api_keys').select('*').order('priority', { ascending: true });
...
const getServiceKey = async (pool, filterFn = null) => {
  ...
  if (!poolKeys || poolKeys.length === 0) {
    console.warn(`[API Keys Service] No healthy keys in pool '${pool}'${filterFn ? ' with filter' : ''}. Falling back to process.env.`);
    // Fallback logic
    if (pool === 'vision') {
      return { api_key: process.env.GEMINI_API_KEY, model_id: 'gemini-1.5-flash', provider: 'env' };
    }
    if (pool === 'voice') {
      // If filtering for TTS vs STT
      return { api_key: process.env.GROQ_API_KEY, model_id: 'whisper-large-v3-turbo', provider: 'env' };
    }
    if (pool === 'weather') {
      return { api_key: process.env.WEATHER_API_KEY, provider: 'env' };
    }
    return null;
  }
