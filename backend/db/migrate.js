...
async function runMigrations() {
  ...
  try {
    ...
    if (error && error.code === "PGRST204") {
      ...
      console.warn(
        "[DB] → Please run backend/db/supabase-schema.sql in the Supabase SQL Editor.",
      );
      return;
    }
...
async function runColumnMigrations() {
  ...
  for (const sql of migrations) {
    try {
      ...
      // Column may already exist or rpc not available — try direct query
      try {
        await supabase.from("_migrations_check").select("1").limit(0);
      } catch {}
    }
  }
...
async function seedDefaults() {
  ...
  // ── Outbreaks ─────────────────────────────────────────────────
  const { count: outbreakCount } = await supabase
    .from("outbreaks")
    .select("*", { count: "exact", head: true });
  if (!outbreakCount) {
